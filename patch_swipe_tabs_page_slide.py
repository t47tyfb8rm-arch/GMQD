...
VERSION_TO = "20260728-v1-swipe-tabs-page-slide"


CSS_PATCH = """
        /* github patch: swipe tabs page slide */
        .main,
        .product-grid {
            overflow-x: hidden !important;
        }
        .product-grid.swipe-anim-left {
            animation: pageSlideFromRight 0.32s ease-out both !important;
        }
        .product-grid.swipe-anim-right {
            animation: pageSlideFromLeft 0.32s ease-out both !important;
        }
...
OLD_TIMEOUTS = (
    ...
    "window.setTimeout(() => grid.classList.remove(directionClass), 260);",
)
NEW_TIMEOUT = "window.setTimeout(() => grid.classList.remove(directionClass), 340);"
...
def main() -> None:
    ...
    if "github patch: swipe tabs page slide" not in html:

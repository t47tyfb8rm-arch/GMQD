...
VERSION_TO = "20260728-v1-detail-exact-three-row"


CSS_PATCH = """
        /* github patch: detail exact three row */
        ...
        .detail-modal .detail-img-area {
            flex: 0 0 auto !important;
            width: 100% !important;
            height: min(21vh, 136px) !important;
        ...
        .detail-modal .detail-row-3 {
            display: grid !important;
            grid-template-columns: repeat(3, minmax(0, 1fr)) !important;
            gap: 7px !important;
            width: 100% !important;
        }
        .detail-modal .detail-mini-card,
        .detail-modal .detail-wide-card {
        ...
        .detail-modal .detail-wide-card .detail-label {
        ...
        .detail-modal .detail-wide-card .detail-value {
        ...
        body.dark .detail-modal .detail-mini-card,
        body.dark .detail-modal .detail-wide-card {
            background: #231e23 !important;
            border-color: rgba(255,255,255,.08) !important;
        }
        body.dark .detail-modal .detail-value {
        ...
        @media (max-width: 400px) {
            ...
            .detail-modal .detail-img-area {
                height: min(19vh, 118px) !important;
            }
            .detail-modal .detail-title {
...
NEW_OPEN_DETAIL = r"""    function openDetail(id) {
        ...
        const noteHtml = r.note ? `
            <li>
                <div class="detail-wide-card">
                    ...
                    <span class="detail-value">${escapeHtml(r.note)}</span>
                </div>
            </li>` : '';
        list.innerHTML = `
            <li>
                <div class="detail-row-3">
                    <div class="detail-mini-card"><span class="detail-label">品牌</span><span class="detail-value">${escapeHtml(r.brand || '—')}</span></div>
                    <div class="detail-mini-card"><span class="detail-label">尺码</span><span class="detail-value">${escapeHtml(r.orderNo || '—')}</span></div>
                    <div class="detail-mini-card"><span class="detail-label">数量</span><span class="detail-value">${r.quantity || 1}</span></div>
                </div>
            </li>
            <li>
                <div class="detail-row-3">
                    <div class="detail-mini-card"><span class="detail-label">状态</span><span class="detail-value">${escapeHtml(r.status || '—')}</span></div>
                    <div class="detail-mini-card"><span class="detail-label">渠道</span><span class="detail-value">${escapeHtml(r.platform || '—')}</span></div>
        ...
        // 20260728 exact three row marker
    }
"""
...
def main() -> None:
    ...
    if "github patch: detail exact three row" not in html:
        html = html.replace("    </style>", CSS_PATCH + "\n    </style>", 1)

    if "20260728 exact three row marker" not in html:
